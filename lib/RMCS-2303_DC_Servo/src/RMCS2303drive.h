...
class RMCS2303
{
  public:
    ...
    // Removed Serial_selection as we only support HardwareSerial now
    
    void begin(HardwareSerial *hport, long baudrate);
    // Removed SoftwareSerial begin
  ...
  private:
    HardwareSerial *port; // Store pointer to the hardware serial port
};
#endif